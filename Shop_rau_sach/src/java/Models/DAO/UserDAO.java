    ...
    public boolean signUp(User bean) {
        ...
        if (!search(bean.getuName())) {
            try {
                ...
                String sql = "INSERT INTO `users` (`uId`, `uName`, `uEmail`, `uPhone`, `uAddress`, `uJob`, `uPassword`, `uRole`, `uPhoto`, `uCreateDate`) VALUES (NULL,?,?,?,?,?, MD5(?), ?, ?, ?)";
                ...
                pst1.setString(3, bean.getuPhone());
                pst1.setString(4, bean.getuAddress());
                pst1.setString(6, bean.getuPassword());
                pst1.setString(5, bean.getuJob());
                pst1.setString(7, bean.getuRole());
                pst1.setString(8, "upload/user.png");
                pst1.setDate(9, new Date(time));
                pst1.executeUpdate();
                b = true;
...
//                con = db.getConnect();
                ...
                if (rs.next()) {
                    userSinIn = new User(rs.getString("uName"),rs.getString("uEmail"), rs.getInt("uId"),rs.getString("uJob"), rs.getString("uAddress"), rs.getString("uPhoto"), rs.getString("uRole"), rs.getString("uPhone"),rs.getString("uPassword"), rs.getDate("uCreateDate"));
                }
                return userSinIn;
            }
               closeConnection();
    ...
    public boolean updateUser(User updateUser ,String path) {
        ...
        try {
            ...
            if (search(updateUser.getuName())) {
                ...
                PreparedStatement pst = con.prepareStatement("UPDATE `users` SET `uName`=?,`uEmail`=?,`uAddress`=?,`uJob`=?,`uPassword`=?,`uRole`=?,`uPhoto`=?, `uPhone`=? WHERE `uId`= ?");
                ...
                pst.setString(6, updateUser.getuRole());
                pst.setString(7, updateUser.getuPhoto());
                pst.setString(8, updateUser.getuPhone());
                pst.setInt(9, updateUser.getuId());
...
//                user.setuCash(rs.getInt("uCash"));
    ...
    public boolean updateUserBalance(User updateUser) throws SQLException {
            ...
            if (search(updateUser.getuName())) {
                ...
                PreparedStatement pst = con.prepareStatement("UPDATE `users` WHERE `uId`=?");
//                pst.setDouble(1, updateUser.getuCash());
                pst.setInt(1, updateUser.getuId());       
                pst.executeUpdate();
                   closeConnection();
                return true;
            }
